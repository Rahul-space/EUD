...
# # from ai.diagnostics_agentic import AIAgenticEngine




# # from ai.diagnostics import register_diagnostic_routes
# # register_diagnostic_routes(app, scheduler)
...
#     app = FastAPI(title="SysAssist", version="2.2.0", docs_url=None, redoc_url=None)
...
#         diagnosis = ""
#         try:
#             diagnosis = ai_engine.analyze_context(context)
#         except Exception:
#             pass
#         alert_manager.process_violations(critical_violations, diagnosis=diagnosis)

#     scheduler.state.alert_callbacks.append(_on_violations)

#     # ── UI serving ─────────────────────────────────────────────────────────
#     # Load once at startup — cached in closure
...
#                 None, lambda: ai_engine.chat(msg.message, context, msg.history)
#             )
...
#         ctx = scheduler.state.latest_context
#         if ctx is None:
#             return {"analysis": "System is initialising — please wait.", "timestamp": time.time()}
#         try:
...
#             result = await asyncio.get_event_loop().run_in_executor(
#                 None, lambda: ai_engine.set_claude_key(req.api_key, model)
#             )
#         elif req.provider == "openai":
#             model  = req.model or "gpt-4o-mini"
#             result = await asyncio.get_event_loop().run_in_executor(
#                 None, lambda: ai_engine.set_openai_key(req.api_key, model)
#             )
#         elif req.provider == "nvidia":
#             model  = req.model or "nvidia/nemotron-3-super-120b-a12b"
#             result = await asyncio.get_event_loop().run_in_executor(
#                 None, lambda: ai_engine.set_nvidia_key(req.api_key, model)
#             )
#         else:
...
#                 None, lambda: ai_engine._infer_single(prompt)
#             )
...
# # from ai.diagnostic import AIDiagnosticEngine
# from ai.diagnostics_agentic import AIDiagnosticEngine
...
#         """
#         context = scheduler.state.latest_context
#         # Push a "thinking" step so the UI shows live status
#         await ws_mgr.broadcast({"type": "agent_thinking", "step": "Calling live system tools…"})
#         try:
#             response = await ai_engine.chat_async(msg.message, context, msg.history)
#             return {"response": response, "timestamp": time.time()}
#         except Exception as exc:
...
#         """
#         Agentic full-system analysis (Analyze Now button).
#         Diagnostician uses live tools — not just a context snapshot.
#         """
#         ctx = scheduler.state.latest_context
#         if ctx is None:
#             return {"analysis": "System is initialising — please wait a few seconds.",
#                     "timestamp": time.time()}
#         await ws_mgr.broadcast({"type": "agent_thinking", "step": "Investigating system…"})
#         try:
#             analysis = await ai_engine.analyze_context_async(ctx)
#             return {"analysis": analysis, "timestamp": time.time()}
#         except Exception as exc:
...
from config.settings import settings
from agent.scheduler import MetricScheduler
# from ai.diagnostic import AIDiagnosticEngine, _snapshot, _DONE
...
def create_app(scheduler: MetricScheduler, alert_manager: AlertManager) -> FastAPI:
    ...
    async def execute_remediation(req: dict):
        """
        User-approved remediation endpoint — called from the chat window.
        Payload: { alert_id, action, approved }
        Maps the action name to the appropriate RemediationEngine call.
        """
        if not req.get("approved"):
            return {"success": False, "message": "Remediation not approved by user."}

        action    = req.get("action", "")
        alert_id  = req.get("alert_id", "")

        # Acknowledge the alert so the state machine progresses
        try:
        ...
        if action == "kill_top_process":
            # Kill the highest CPU process that isn't a protected system process
            BLOCKED = {"system", "svchost", "wininit", "csrss", "lsass", "smss"}
            procs   = scheduler.state.latest_processes or []
            target  = next(
                (p for p in procs if p.name.lower() not in BLOCKED and p.cpu_percent > 5),
                None
            )
            if target:
                r = remediation.kill_process(target.pid, force=False)
                return {"success": r.success, "message": r.message, "details": r.details}
            return {"success": False, "message": "No eligible high-CPU process found."}
            ...
            r = await loop.run_in_executor(None, remediation.clear_temp_files)
            return {"success": r.success, "message": r.message, "details": r.details}

        elif action == "reset_network":
            r = await loop.run_in_executor(None, remediation.reset_network)
            return {"success": r.success, "message": r.message, "details": r.details}
            ...
            r = await loop.run_in_executor(None, remediation.restart_service, svc)
            return {"success": r.success, "message": r.message, "details": r.details}
    ...
    async def health_check():
        """Lightweight ping used by the chat window to test connectivity."""
    ...
    async def approve_proposal(session_id: str):
        """
        User approves a remediation proposal.
        The Remediator agent executes the plan and reports results via WebSocket.
        """
        ok = ai_engine.approve_proposal(session_id)
        if ok:
            proposal = ai_engine._proposals.get(session_id)
    ...
    async def clear_chat_session():
        """Clear conversation memory and pending proposals — start fresh."""
        ai_engine.clear_session()
        return {"success": True, "message": "Session cleared — memory reset"}
    ...
    async def set_api_key(req: SetAPIKeyReq):
        """
        Configure an external AI provider (claude, openai, nvidia).
        API key is held in memory only — never written to disk.
        """
        result = await asyncio.get_event_loop().run_in_executor(
            None,
            lambda: ai_engine.set_api_key(req.provider, req.api_key, req.model)
        )
        return result
    ...
    async def analyze_tab(tab: str):
        ...
        # ── Send tab-specific context to the agent ────────────────────────
        try:
            # Inject the pre-loaded snapshot so the agent knows the basics,
            # then add the tab-specific domain data that was built above.
            snapshot_text = _snapshot.text()
            combined_task = (
                f"{snapshot_text}\n\n"
                f"=== {tab_name.upper()} TAB DATA ===\n"
                f"{tab_context}\n\n"
                f"Analyze ONLY the {tab_name} data above.\n"
                f"Respond ONLY in valid JSON with no markdown fences:\n"
                '{"root_cause":"...","warning_level":"critical|warning|info|healthy",'
                '"warning_score":0,"impacted_components":[],'
                '"recommended_actions":[],"preventive_suggestions":[],"summary":"...'
                f'"}}\n{_DONE}'
            ...
            if ai_engine._model_client:
                # Use a fresh agent with the tab-specific system prompt
                from autogen_agentchat.agents import AssistantAgent
                from autogen_agentchat.teams import RoundRobinGroupChat
                from autogen_agentchat.conditions import TextMentionTermination, MaxMessageTermination
                from ai.diagnostic import _DONE as _D

                tab_agent = AssistantAgent(
                    name           = f"{tab_name}Analyst",
                    model_client   = ai_engine._model_client,
                    system_message = system_prompt,
                    tools          = ai_engine._read_tools,
                )
                term   = TextMentionTermination(_D) | MaxMessageTermination(12)
                tteam  = RoundRobinGroupChat([tab_agent], termination_condition=term)
                tresult = await tteam.run(task=combined_task)
                raw = ai_engine._extract_reply(tresult)
            else:
                # Fallback: single-shot Ollama with tab prompt
                raw = await asyncio.get_event_loop().run_in_executor(
                    None, lambda: ai_engine._fallback_tab(system_prompt, tab_context)
                )

            if raw:
                raw = raw.replace(ai_engine.__class__.__module__ and
                                  __import__("ai.diagnostic", fromlist=["_DONE"])._DONE or
                                  "COGNIX_DONE", "").strip()
                match = _re2.search(r"\{.*\}", raw, _re2.DOTALL)
                if match:
                    result = _j2.loads(match.group())
    ...
    # ── Approved plan execution (Remediator agent) ──────────────────────────
    async def _execute_approved_plan(engine, proposal, session_id: str):
        """
        Runs Agent 2 (Remediator) to execute an approved remediation plan.
        Result is broadcast via WebSocket as agent_complete.
        """
        try:
            result_text = await engine.execute_approved_plan_async(proposal, session_id)
            await engine._push({
                "type":       "agent_complete",
                "session_id": session_id,
                "result":     result_text,
            ...
            logger.error(f"[server] _execute_approved_plan error: {e}")
            await engine._push({
                "type":       "agent_error",
                "session_id": session_id,
                "message":    str(e)[:300],
